...
def pl_outer_join(df1: pl.DataFrame, df2: pl.DataFrame, how, left_on, right_on):
    ...
    # perform join. Newer polars eliminates the right-side join key after the
    # join, so only the left ``_tmp`` column is guaranteed to remain.
    joined = df1.join(df2, left_on=new_names[0], right_on=new_names[1], how=how)
    cols_to_drop = [c for c in new_names if c in joined.columns]
    if cols_to_drop:
