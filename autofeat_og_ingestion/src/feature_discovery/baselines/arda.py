...
def select_arda_features_budget_join(
    ...
    while len(nodes) > 0:
        ...
        # Prepare data. Extract ``y`` before AutoML feature generation
        # because the generator may drop the label column (e.g. when it is
        # typed as a binary indicator like ``target``/``Class``).
        y = joined_tables_batch[target_column].copy()
        feature_batch = joined_tables_batch.drop(columns=[target_column])
        X = AutoMLPipelineFeatureGenerator(
            enable_text_special_features=False, enable_text_ngram_features=False
        ).fit_transform(X=feature_batch).fillna(0)
        # Align label index with the (possibly reordered/filtered) feature rows.
        y = y.loc[X.index]
